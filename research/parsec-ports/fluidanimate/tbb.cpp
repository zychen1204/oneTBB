...
// oneTBB port: the old tbb::task spawning API (allocate_root/allocate_child,
// spawn_root_and_wait, task_list) was removed. The worker classes below keep
// their original shape via this minimal base; the two launcher levels are
// replaced by LaunchGrids(), which reproduces the same NUM_GRIDS x NUM_TASKS
// task decomposition with tbb::task_group.
namespace tbbport {
struct task {
...
// oneTBB port of GridLauncher/WorkerLauncher: one task per (grid, task-id)
// pair, identical decomposition to the original two-level task tree.
template <class T>
static void LaunchGrids(int num_grids) {
  tbb::task_group outer;
  for (int i = 0; i < num_grids; ++i) {
    outer.run([i] {
      const int sz = grids[i].sz;
      const int ez = grids[i].ez;
      tbb::task_group inner;
      for (int t = 1; t < NUM_TASKS; ++t)
        inner.run([t, i, sz, ez] { T w(t, i, sz, ez); w.execute(); });
      { T w(0, i, sz, ez); w.execute(); }
      inner.wait();
    });
  }
  outer.wait();
...
class ClearParticlesMTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
class RebuildGridMTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
class InitDensitiesAndForcesMTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
class ComputeDensitiesMTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
class ComputeDensities2MTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
class ComputeForcesMTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
class ProcessCollisionsMTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
class ProcessCollisions2MTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
class AdvanceParticlesMTWorker: public tbbport::task {
  int tid;
  int pid;
  int start;
  int end;
...
void AdvanceFrame()
{
  ...
  LaunchGrids<ClearParticlesMTWorker>(NUM_GRIDS);
  LaunchGrids<RebuildGridMTWorker>(NUM_GRIDS);
  LaunchGrids<InitDensitiesAndForcesMTWorker>(NUM_GRIDS);
  LaunchGrids<ComputeDensitiesMTWorker>(NUM_GRIDS);
  LaunchGrids<ComputeDensities2MTWorker>(NUM_GRIDS);
  LaunchGrids<ComputeForcesMTWorker>(NUM_GRIDS);
  LaunchGrids<ProcessCollisionsMTWorker>(NUM_GRIDS);
  LaunchGrids<AdvanceParticlesMTWorker>(NUM_GRIDS);

#if defined(USE_ImpeneratableWall)
  // N.B. The integration of the position can place the particle
  // outside the domain. We now make a pass on the perimiter cells
  // to account for particle migration beyond domain.
  LaunchGrids<ProcessCollisions2MTWorker>(NUM_GRIDS);
#endif
}
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  tbb::global_control tbb_gc(tbb::global_control::max_allowed_parallelism, threadnum);
#ifdef ENABLE_VISUALIZATION
