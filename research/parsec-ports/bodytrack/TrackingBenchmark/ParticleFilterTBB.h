...
// oneTBB port: tbb::pipeline/filter removed; see TrackingModelTBB.h.
template<class T>
class ParticleFilterTBB : public ParticleFilter<T> {
...
public:
	//TBB pipeline stage function (driven by the stage loop in main since the oneTBB port)
	void *operator()(void *token);

	ParticleFilterTBB() : mOutputBMP(false), mFrame(0) {};
