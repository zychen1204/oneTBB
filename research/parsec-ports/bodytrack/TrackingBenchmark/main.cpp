...
{
	// oneTBB port: task_scheduler_init was removed; global_control caps concurrency.
	tbb::global_control* tbb_gc = nullptr;
	...
	{	tbb_gc = new tbb::global_control(tbb::global_control::max_allowed_parallelism, threads);
		cout << "Number of Threads : " << threads << endl;
	}
	...
	// oneTBB port: tbb::pipeline was removed. The original ran the two stages
	// with pipeline.run(1), i.e. at most one token in flight and therefore no
	// stage overlap, so a sequential stage loop is exactly equivalent.
	while (void* token = model(nullptr)) {
		pf(token);
	}
